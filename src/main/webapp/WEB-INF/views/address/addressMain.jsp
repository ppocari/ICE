...
<section id="addressMain_section">
	<!-- post방식으로 페이징 처리 -->
	<form action="<c:url value='/address/addressMain.do'/>" name="frmPage"
		method="post">
		<input type="hidden" name="currentPage"> <input type="hidden"
			name="searchCondition" value="${param.searchCondition}"> <input
			type="hidden" name="searchKeyword" value="${param.searchKeyword}">
		<input type="hidden" name="nextKeyword" value="${param.nextKeyword}">
	</form>
					...
					<form name="addressList" method="post">
						<div
							...
							<div style="float: right">
								...
								<button type="submit" class="btn btn-info"
									formaction="<c:url value='/addressExcel.do' />">주소
									내보내기</button>
						...
						<div
							...
							<div id="buttonsDiv">
								<div id="divAll">
									<div id="divLeft">
										<input type="button" id="btAdd" value="추가"> <input
											type="button" id="btDel" value="휴지통으로 이동">
									</div>
									<span> <a
										href="<c:url value='/address/addressMain.do?searchCondition=all'/>">전체</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=가&nextKeyword=나'/>">가</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=나&nextKeyword=다'/>">나</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=다&nextKeyword=라'/>">다</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=라&nextKeyword=마'/>">라</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=마&nextKeyword=바'/>">마</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=바&nextKeyword=사'/>">바</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=사&nextKeyword=아'/>">사</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=아&nextKeyword=자'/>">아</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=자&nextKeyword=차'/>">자</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=차&nextKeyword=카'/>">차</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=카&nextKeyword=타'/>">카</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=타&nextKeyword=파'/>">타</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=파&nextKeyword=하'/>">파</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=kor&searchKeyword=하'/>">하</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=eng'/>">영어</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=num'/>">숫자</a>
										<a
										href="<c:url value='/address/addressMain.do?searchCondition=favorite'/>">즐겨찾기</a>
									</span>
								</div>
							</div>
						</div>
							...
							<table id="tableForm">
								...
								<c:if test="${!empty param.searchCondition}">
									...
									<c:if test="${pagingInfo.totalRecord==0 }">
										<!-- 등록된 주소가 없음 -->
										<tr>
											<td colspan="8">조건에 맞는 주소가 없습니다.</td>
										</tr>
									</c:if>
								</c:if>
								<c:if test="${pagingInfo.totalRecord !=0 }">
									...
									<c:forEach var="address" items="${adList }">
										<tr>
											<td><input name="adItems[${idx }].adNo"
												value="${address.adNo }" type="checkbox" class="addressCB"></td>
											<td class="star" id="${address.adNo }_${address.isFavorite}"
												style="cursor: pointer">☆</td>
											<td><a
												href="<c:url value='/address/detailAddress.do?adNo=${address.adNo }'/>"
												class="addressName">${address.name}</a></td>
